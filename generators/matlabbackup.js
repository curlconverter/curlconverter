...
function callFunction (outputVariable, functionName, params, noNewLineBefore) {
  let result = !noNewLineBefore ? '\n' : ''
...
function toHTTPInterface (request) {
  ...
  response += callFunction('uri', 'URI', uriParams)
  let options = ''
  let credentials = ''
  if (request.auth) {
    ...
    credentials += callFunction('cred', 'Credentials', authparams)
    // credentials += `\ncred = Credentials(${authparams});`
    response += credentials
  }
  if (credentials) {
    options += callFunction('options', 'HTTPOptions', ['\'Credentials\'', 'Cred'])
